...
`objective`     |     Character giving the objective function to optimize. Possible values are: `"LL"` for the Log-Likelihood, `"LOO"` for the Leave-One-Out sum of squares, `"LMP"` for the Log-Marginal Posterior, and `"VLL"` or `"VLL(m)"` for the [Vecchia approximated log-likelihood](vecchia.Kriging) with `m` conditioning neighbors (default 30), recommended for large designs in low dimension.
